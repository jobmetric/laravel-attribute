...
@section('body')
    <x-list-view name="{{ $label }}" action="{{ $route }}">
        <x-slot name="filter">
            <div class="col-md-3">
                <div class="mb-5">
                    <label class="form-label">{{ trans('attribute::base.list.attribute_value.filters.name.title') }}</label>
                    <input type="text" name="translation[name]" class="form-control filter-list" id="filter-name" placeholder="{{ trans('attribute::base.list.attribute_value.filters.name.placeholder') }}" autocomplete="off">
                </div>
            </div>
        </x-slot>
        ...
        <thead>
            <tr>
                ...
                <th width="58%" class="text-gray-800 auto-width-content">{{ trans('package-core::base.list.columns.name') }}</th>
